...
export async function analyzeMacro(symbol, meta, assetContext, dailyBars = null) {
  ...
  //
  // When the caller already holds daily bars there is nothing to fetch: the
  // daily series carries ten years of closes and this needs three. Re-fetching
  // was doubling the upstream calls for every analysis, which matters most in
  // a scan - 61 symbols was making ~97 requests where ~79 would do, and Yahoo
  // starts throttling long before it starts erroring.
  let assetCloses = null;
  if (dailyBars?.length) {
    assetCloses = {};
    for (const b of dailyBars) {
      assetCloses[new Date(b.time).toISOString().slice(0, 10)] = b.close;
    }
  } else {
    try {
